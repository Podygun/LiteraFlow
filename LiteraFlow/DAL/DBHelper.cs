...
public static class DBHelper
{
    ...
    /// <summary>
    /// Select collection of rows with params
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <param name="sql"></param>
    /// <param name="model"></param>
    ...
    /// <typeparam name="T"></typeparam>
    /// <returns></returns>
    public static async Task<List<T>> QueryCollectionAsync<T>(string sql)
    {
        using (var con = new NpgsqlConnection(ConString))
        {
            await con.OpenAsync();
    ...
    public static async Task<int?> AddBookWithTransactionAsync(string sqlInsertBook, BookModel book, string sqlInsertAuthorBook, int profileId)
    {
        using (var con = new NpgsqlConnection(ConString))
        {
            await con.OpenAsync();
            using(var transaction = con.BeginTransaction())
            {
                ...
                {
                    //add book
                    int bookId = await ExecuteScalarAsync<int>(sqlInsertBook, book);

                    //add many-to-many relation to AuthorsBooks Table
                    await ExecuteAsync(sqlInsertAuthorBook, new { bookid = bookId, profileid = profileId });
